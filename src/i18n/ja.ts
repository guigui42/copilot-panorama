...
export const ja: Translations = {
  ...
  tipsViz: {
    ...
    perStep: 'ステップあたり',
    steps: 'ステップ',
    accuracy99: 'ステップあたり99%',
    accuracy95: 'ステップあたり95%',
    bePrecise: '正確に',
  ...
  tipsComponents: {
    ...
    guardrails: {
      name: 'ガードレールを設定する',
      description: 'エージェントフローはゴール達成まで反復する — 停止シグナルでループを区切る',
      details:
        'エージェントは設計上反復します：計画 → 編集 → ツール実行 → 修正 → 繰り返し。' +
        '制限がなければ、エージェントは成功する（またはコンテキストを使い果たす）まで' +
        '続けます。明示的な停止シグナルを追加しましょう：「最初のテスト成功で停止」' +
        '「代替案は最大2つまで」「最初のアプローチが失敗したら理由を説明して停止」。' +
        '決定論的ガードレール（テスト、リンター）と組み合わせることで、トークンを' +
        '消費し続ける暴走セッションを防ぎます。',
    ...
    'prompt-anatomy': {
      ...
      details:
        '効果的なプロンプトの信頼できる3つの構成要素：(1) 正確に — 望ましい結果を含めて、' +
        '変更を明瞭で曖昧さのない言葉で記述。(2) 停止シグナルを追加 — 「最初のテスト成功で' +
        '停止」「無関係なコードをリファクタしない」。(3) 既知のコンテキストを事前に — ' +
        '関連するファイル、フォルダ、ドキュメントを指定し、エージェントが検索でトークンを' +
        '無駄にしないようにする。プロンプトはステアリングホイールです；この3つの要素が' +
        'エージェントを道から外させません。',
      useCases: ['Chat', 'Agent Mode', 'CLI'],
    },
    'context-rot': {
      name: 'コンテキストロット',
      description: '巨大なコンテキストウィンドウでも劣化する — 中央のトークンが見失われ、直近が支配する',
      details:
        '200kウィンドウのモデルだからといって、埋め尽くすべきではありません。文書化された' +
        '2つの失敗モード：「Lost in the Middle」— 長いコンテキストの中央に置かれたトークンは' +
        '冒頭や末尾のトークンよりも信頼性が低く想起されます。「直近バイアス」— ウィンドウが' +
        '約50%を超えると、モデルは直近のトークンに大きく依存します。緩和策：プロンプトを' +
        '短く保ち、最重要な指示を冒頭または末尾に置き、劣化が始まる前に新しいスレッドを開始する。',
      useCases: ['長時間セッション', '大規模コードベース', 'マルチファイル編集'],
    },
    'think-in-code': {
      name: 'コードで考える',
    ...
    'trim-shell-outputs': {
      ...
      details:
        ...
        'うるさいコマンドをトリマー（例：github.com/rtk-ai/rtk）でラップして、エージェントが' +
        '関連する末尾だけを見るようにしましょう：エラー、警告、最終サマリー。これにより' +
        '次ループのキャッシュ入力が劇的に縮小し、「中央で見失う」効果も減って想起力が向上します。',
    ...
    'iterate-configs': {
      ...
      details:
        ...
        '二度起きないようにします。/chronicleを定期的に実行してパターンを表面化させましょう。' +
        '時間とともに、タスクごとに余計な作業をすることなく、エージェントが目に見えて信頼できる' +
        'ようになります。',
